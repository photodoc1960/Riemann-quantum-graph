...
\end{equation}
Finally, let $\hat{R}_2(r)$ be the empirical two-point pair-correlation density of $\{\tilde{k}_j\}$ (all pairs with separation $r \in (0, 3]$, binned uniformly into $50$ bins of width $\Delta r$), and $R_2^{\mathrm{GUE}}(r)$ the GUE two-point correlation function normalized to the same domain. Then
\begin{equation}
    \mathcal{S}_{\mathrm{corr}} = \exp\!\left(-\sum_b \bigl(\hat{R}_2(r_b) - R_2^{\mathrm{GUE}}(r_b)\bigr)^2 \Delta r\right).
\end{equation}
Scoring is validated against two reference cases: self-comparison (the Riemann zeros scored against themselves) yields $\mathcal{S} = 0.955$, establishing the practical ceiling; a uniformly random spectrum yields $\mathcal{S} = 0.144$, establishing the floor.
...
\begin{tabular}{@{}lllc@{}}
\hline
Section & Experiment & Scattering & Optimizer \\
\hline
\ref{sec:results-topology} & $7$-vertex enumeration ($457$ graphs) & TRS-broken Neumann & DE \\
\ref{sec:results-neumann} & Cycle $C_n$ Neumann ceiling & Pure Neumann & CMA-ES \\
\ref{sec:results-neumann} & Cycle $C_n$ TRS-broken ceiling & TRS-broken Neumann & CMA-ES \\
\ref{sec:results-U2} & $C_7$, cycle-size scaling $C_5$--$C_{21}$ & U(2) & CMA-ES \\
\ref{sec:results-topology-indep} & Theta $\Theta_k$ topology comparison & U(2), U(3) & CMA-ES \\
\ref{sec:results-surrogate} & GUE surrogate control ($K=50$) & U(2) on $C_7$ & CMA-ES \\
\hline
\end{tabular}
\caption{Mapping of manuscript experiments to graph family, scattering-matrix family, and black-box optimizer. All pipelines are followed by a Nelder--Mead polish.}
...
We report the surrogate analysis as a descriptive comparison rather than as a formal hypothesis test, because the underlying pipeline is a stochastic adaptive optimizer and constructing a valid end-to-end null distribution over the joint space of (target sequence, optimizer trajectory, restart best) is not attempted here. At the $K = 50$ sample size the optimizer reaches MAD $= 0.260 \pm 0.035$ mean spacings across surrogates (mean $\pm$ standard deviation over $K = 50$ per-surrogate best-of-restart MAD values), with minimum $0.206$, maximum $0.377$, median $0.259$. The Riemann result at MAD $= 0.147$ lies below the minimum of all $50$ surrogate MADs; its empirical rank against the surrogate distribution is therefore $0/50$. The standardized deviation of the Riemann MAD from the surrogate mean, expressed in surrogate-standard-deviation units, is $Z = (0.260 - 0.147)/0.035 = 3.21$. We report $Z$ in place of Cohen's $d$ because the Riemann side is a single point rather than a second sample distribution, so a $d$-statistic is not well-defined here; the numerical value coincides with what one would compute from the naive plug-in formula and is included solely as a magnitude of separation. The observation that $Z > 3$ and rank $= 0/50$ is descriptive: it states that the observed Riemann outcome is far outside the surrogate distribution as summarized. It is not a $p$-value against a specified null, and we do not claim conventional significance thresholds. Figure~\ref{fig:surrogateHist} shows the surrogate MAD distribution with the Riemann baseline overlaid.
...
{\sloppy All surrogate seeds, per-surrogate optimization records, and aggregate statistics are deposited in the files \texttt{gue\_surrogate\_control.jsonl} and \texttt{gue\_surrogate\_control\_summary.json} under \texttt{results/} in the public repository.\par}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{results/figure4_surrogate_distribution.png}
\caption{GUE surrogate MAD distribution versus the Riemann baseline. Histogram of best-of-restart MADs across $K = 50$ independent GUE surrogate sequences unfolded to the Riemann--von Mangoldt smooth density (blue). Vertical red line: Riemann MAD $= 0.147$. Dashed line: surrogate mean. Gray band: surrogate mean $\pm$ standard deviation. No surrogate reaches the Riemann value; the Riemann outcome lies at empirical rank $0/50$, at $Z = 3.21$ standardized surrogate-standard-deviation units below the surrogate mean. Reported as descriptive statistics, not as a formal hypothesis test (Section~\ref{sec:results-surrogate}).}
...
reprinted in H. Weber (ed.),
\textit{Bernhard Riemann's Gesammelte Mathematische Werke},
2nd ed., Teubner, Leipzig (1892), pp.~145--155;
English translation in H.~M. Edwards,
\textit{Riemann's Zeta Function},
Academic Press (1974), Appendix.
% MR: MR0466039 (Edwards)
